...
export default function ItemsProvider({ children }: Children) {
  ...
  const {
    ...
    `/item?category=${selectedCategory}&subcategory=${selectedSubcategory}&type=${selectedAccessType}&liked=${selectedLiked}&page=${selectedPage}`,
    getData,
  );
  const {
    data: dataFavorite,
  ...
  } = useSWR(userId && token ? `/favorite?userId=${userId}` : null, (url) =>
    getData(url, token),
  );
  ...
  const handleFilters = (value: string, type: "popularity" | "access") => {
    switch (type) {
      case "popularity":
  ...
  const values: ContextItemsValues = {
    items,
    pages,
    token,
    userId,
