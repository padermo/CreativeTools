...
export default function InputReusable({id, error, value, placeholder, type, onChange, onPaste}:InputReusableProps){
  switch(type){
    ...
    case 'area': 
      return (
        <ConfigThemeAnt>
          <Input.TextArea id={id} rows={4} placeholder={placeholder} value={value} onChange={onChange} style={{borderColor: error && '#BC2929', resize:'none'}} className='w-full rounded-md border p-2 text-[#222] dark:text-white dark:placeholder:text-gray-400'/>
        </ConfigThemeAnt>
      )
    default:
      break;
