...
export default function Account() {
  ...
  return (
    <article className='w-full min-h-dvh max-h-full flex flex-col px-4 lg:justify-normal lg:flex-row md:px-6 lg:px-8'>
      ...
      <div className='w-full h-dvh flex flex-col justify-center items-center text-center gap-6 text-wrap text-lg font-light tracking-wide'>
        <h3 className='w-full md:w-1/2 lg:w-1/2'>{t('title')}</h3>
        <div>
          <ButtonReusable text={t('cta')} htmlType='button' type='primary' loading={false} onClick={() => router.push('/auth/register')} />
        </div>
      </div>
    </article>
  )
